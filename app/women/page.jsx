    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                ...
                // Show Women specific and Unisex items
                setProducts(data.filter(p => p.section === 'women' || p.section === 'unisex'));
            } catch (error) {
        ...
        <div className="page" style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
            ...
            <main className="container" style={{ padding: "1.5rem 0", flex: 1, maxWidth: "1400px", margin: "0 auto" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", padding: "0 1rem", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
                    <div>
                        <h1 style={{ fontSize: "1.5rem", fontWeight: "800", color: "var(--text)", margin: 0 }}>Women's Collection</h1>
                        <p style={{ color: "var(--muted)", fontSize: "0.85rem", margin: "0.2rem 0 0 0" }}>Engineered elegance for her.</p>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="product-grid">
                        {sortedProducts.map((product) => (
                            <ProductCard key={product._id} product={product} onAddToCart={addItem} source="women" />
                        ))}
                    </div>
                )}
            </main>
